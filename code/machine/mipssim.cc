...
Machine::OneInstruction(Instruction *instr)
{
    ...
    // Fetch instruction
    if (!machine->ReadMem(registers[PCReg], 4, &raw)) return;  // exception
                                                               // occurred
    ...
    case OP_BEQ:
        ...
        if (registers[instr->rs] ==
            registers[instr->rt]) pcAfter = registers[NextPCReg] + IndexToAddr(
                instr->extra);
        break;
    ...
    case OP_BGEZ:
        ...
        if (!(registers[instr->rs] & SIGN_BIT)) pcAfter = registers[NextPCReg] +
    ...
    case OP_BGTZ:
        ...
        if (registers[instr->rs] >
            0) pcAfter = registers[NextPCReg] + IndexToAddr(instr->extra);
        break;
    ...
    case OP_LBU:
        ...
        if ((value & 0x80) && (instr->opCode == OP_LB)) value |= 0xffffff00;
        else value &= 0xff;
    ...
    case OP_LHU:
        ...
        if ((value & 0x8000) && (instr->opCode == OP_LH)) value |= 0xffff0000;
        else value &= 0xffff;
        nextLoadReg   = instr->rt;
        nextLoadValue = value;
        break;
    ...
    case OP_LWR:
        ...
        if (registers[LoadReg] ==
            instr->rt) nextLoadValue = registers[LoadValueReg];
        else nextLoadValue = registers[instr->rt];
    ...
    case OP_SB:
        ...
        if (!machine->WriteMem((unsigned)
                               (registers[instr->rs] + instr->extra), 1,
                               registers[instr->rt])) return;
    ...
    case OP_SH:
        ...
        if (!machine->WriteMem((unsigned)
                               (registers[instr->rs] + instr->extra), 2,
                               registers[instr->rt])) return;
    ...
    case OP_SLT:
        ...
        if (registers[instr->rs] < registers[instr->rt]) registers[instr->rd] = 1;
        else registers[instr->rd] = 0;
        break;

    case OP_SLTI:

        if (registers[instr->rs] < (int)instr->extra) registers[instr->rt] = 1;
        else registers[instr->rt] = 0;
        break;
    ...
    case OP_SLTIU:
        ...
        if (rs < imm) registers[instr->rt] = 1;
        else registers[instr->rt] = 0;
        break;
    ...
    case OP_SW:
        ...
        if (!machine->WriteMem((unsigned)
                               (registers[instr->rs] + instr->extra), 4,
                               registers[instr->rt])) return;
