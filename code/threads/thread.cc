...
{
    if (stack != NULL)
#ifdef HOST_SNAKE               // Stacks grow upward on the SnakesASSERT(stack[StackSize - 1] == STACK_FENCEPOST);
#else
        ASSERT(*stack == (unsigned long)STACK_FENCEPOST);
#endif // ifdef HOST_SNAKE
}
...
Thread::Sleep()
{
    ...
    while ((nextThread = scheduler->FindNextToRun()) == NULL) interrupt->Idle();  //
                                                                                  // no
                                                                                  // one
                                                                                  // to
                                                                                  // run,
...
{
    stack =
        (unsigned long *)AllocBoundedArray(StackSize * sizeof(unsigned long));
...
Thread::SaveUserState()
{
    for (int i = 0; i < NumTotalRegs;
         i++) userRegisters[i] = machine->ReadRegister(i);
}
